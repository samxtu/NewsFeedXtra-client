...
class country extends Component {
    ...
    componentDidMount(){
        ...
        const This = this;
        // fetch cached data
        caches.match(`https://newsapi.org/v2/top-headlines?country=${this.props.match.params.name}&page=${this.state.cachePage}`).then(function(res) {
            if (!res) throw Error("No data");
            return res.json();
        }).then(function(response) {
            // don't overwrite newer network data
    ...
    UNSAFE_componentWillReceiveProps(nextProps){
        ...
        caches.match(`https://newsapi.org/v2/top-headlines?country=${nextProps.match.params.name}&category=${nextProps.match.params.title}&page=${this.state.cachePage}`).then(function(res) {
            ...
            const This = this;
        // fetch cached data
        caches.match(`https://newsapi.org/v2/top-headlines?country=${nextProps.match.params.name}&page=${this.cachePage}`).then(function(res) {
            if (!res) throw Error("No data");
        ...
        if(this.state.category === 'All'){
            ...
            .catch((err)=>{
                ...
                if(this.state.theError === "Offline: Turn on data for latest headlines."){
                    caches.match('https://newsapi.org/v2/top-headlines?country='+this.state.country+'&page='+this.state.cachePage,{ignoreMethod:true,ignoreVary:true})
                    .then(function(res) {
                        if (!res) throw Error("No data");
                        console.log("we got the good stuff")
                        return res.json();
